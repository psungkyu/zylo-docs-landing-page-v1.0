...
function FeatureCard({ icon, title, description, accent }: FeatureProps) {
  ...
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-lg bg-${accent}-500/10 flex items-center justify-center mb-4 ${accentIconClasses[accent]}`}>
        {icon}
      </div>
      <h3 className="text-lg font-bold font-mono mb-2 text-foreground">{title}</h3>
...
export default function FeaturesSection() {
  ...
  return (
    <section id="technology" className="relative pt-9 lg:pt-24 lg:pb-24 bg-gradient-to-b from-transparent via-amber-500/5 to-transparent">
      <div className="container">
        ...
        <div className="max-w-3xl mb-16">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">
            Built on cutting-edge AI technology. Designed for teams that demand both performance and security.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { 
                ...
                features: ['Custom solutions', 'Advanced security', 'Enterprise support']
              },
            ].map((plan, index) => (
              <div
                key={index}
              ...
              >
                <div className="font-mono font-bold text-blue-400 mb-2">{plan.name}</div>
                ...
                <a href="https://tally.so/r/wgBlOO" target="_blank" rel="noopener noreferrer" className="w-full px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-medium text-sm transition-colors block text-center">
                  Get Started
                </a>
              </div>
            ))}
